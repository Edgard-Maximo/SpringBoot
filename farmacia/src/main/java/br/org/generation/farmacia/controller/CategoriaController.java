...
public class CategoriaController {
	...
	public HttpEntity<List<CategoriaModel>> GetAll() {
		return ResponseEntity.ok(model_repository.findAll()); // OK = 200 }
	...
	public ResponseEntity<List<CategoriaModel>> GetByDescricao(@PathVariable String descricao) {
		return ResponseEntity.ok(model_repository.findAllByDescricaoContainingIgnoreCase(descricao));

	}

	@PostMapping
	public ResponseEntity<CategoriaModel> post(@RequestBody CategoriaModel descricao) {
		return ResponseEntity.status(HttpStatus.CREATED).body(model_repository.save(descricao));

	}

	@PutMapping
	public ResponseEntity<CategoriaModel> put(@RequestBody CategoriaModel descricao) {
		return ResponseEntity.status(HttpStatus.OK).body(model_repository.save(descricao));

	}

	@DeleteMapping("/{id}")
	public void delete(@PathVariable long id) {
		model_repository.deleteById(id);
	}
}
